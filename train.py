...
# Flash Attention 3 not available on Windows; use PyTorch SDPA instead
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, ve, cos_sin, window_size):
        ...
        # PyTorch SDPA expects (B, n_heads, T, head_dim)
        q = q.transpose(1, 2)
        k = k.transpose(1, 2)
        v = v.transpose(1, 2)
...
class MLP(nn.Module):
    def __init__(self, config):
        super().__init__()
        hidden = ((int(8 / 3 * config.n_embd) + 63) // 64) * 64  # ~same param count as 4x ReLU²
